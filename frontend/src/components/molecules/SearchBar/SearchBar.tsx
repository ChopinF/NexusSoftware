...
const getCategoryIcon = (category: string) => {
  ...
  if (normalized.includes("electronic") || normalized.includes("tech")) return <Laptop size={16} />;
  if (normalized.includes("home") || normalized.includes("house")) return <Home size={16} />;
  if (normalized.includes("cloth") || normalized.includes("fashion")) return <Shirt size={16} />;
...
export const SearchBar: React.FC<SearchBarProps> = ({
  ...
  useEffect(() => {
    const handleClickOutside = (event: MouseEvent) => {
      ...
      if (isDropdownOpen && triggerRef.current && !triggerRef.current.contains(target) && menuRef.current && !menuRef.current.contains(target)) {
    ...
    <div className={styles.searchBarContainer}>
      <div className={styles.searchInner}>
        ...
        >
          <span className={styles.selectedIcon}>
            {selectedCategory ? getCategoryIcon(selectedCategory) : <LayoutGrid size={16} />}
          </span>
      ...
      {isDropdownOpen && (
        <ul className={styles.dropdownMenu} ref={menuRef}>
          ...
          >
            <span className={styles.itemIcon}><LayoutGrid size={16} /></span>
            All Categories
          </li>
